...
/**
 * Generates plain-text WhatsApp message content for each scheduled message type.
 * Each type calls Claude with the portfolio context for a rich, emoji-enhanced message.
 */
@Service
class WhatsAppScheduledMessageContentGenerator(
    ...
    private fun generatePortfolioSummary(): String {
        ...
        val prompt = """
            You are a personal investment assistant sending a WhatsApp portfolio summary.
            ...
            Write a WhatsApp-friendly portfolio summary message. Include total value, overall performance, and a brief comment on portfolio health.
        """.trimIndent()
    ...
    private fun generatePerformanceReport(): String {
        ...
        val prompt = """
            You are a personal investment assistant sending a WhatsApp performance report.
            Use *bold* (single asterisk) and emojis for visual hierarchy.
            ...
            Write a WhatsApp-friendly 1-year performance report. Include key metrics, best/worst positions, and a brief forward-looking comment.
        """.trimIndent()
    ...
    private fun generateAllocationCheck(): String {
        ...
        val prompt = """
            You are a personal investment assistant sending a WhatsApp allocation check.
            Use *bold* (single asterisk) and emojis for visual hierarchy.
            ...
            Write a WhatsApp-friendly allocation check. Highlight positions that need attention (significantly over or underweight) and suggest whether rebalancing action is needed.
        """.trimIndent()
    ...
    private fun generateTopMovers(): String {
        ...
        val prompt = """
            You are a personal investment assistant sending a WhatsApp top movers update.
            Use *bold* (single asterisk) and emojis for visual hierarchy.
            ...
            Write a WhatsApp-friendly top movers message. Briefly comment on why these movements might be happening and whether any action is warranted.
        """.trimIndent()
    ...
    private fun callClaude(prompt: String, fallback: String): String {
        return try {
            val result = claudeClient.complete(
                system = "You are a concise, friendly personal investment assistant for WhatsApp. Use plain text with *bold* and emojis. No markdown.",
                userMessage = prompt,
                maxTokens = 600
            )
