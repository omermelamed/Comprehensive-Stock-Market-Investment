...
import type { MessageType, Frequency, ScheduledMessage, ScheduledMessageRequest } from '@/api/whatsappSchedules'
...
export function ScheduleForm({ initial, onSubmit, onCancel, submitting }: Props) {
  ...
  // Sync label placeholder when type changes and label is empty / still default
  useEffect(() => {
    if (!initial && !label) {
