...
import com.investment.domain.TelegramMessageFormatter.DriftEntry
...
class TelegramReadHandlers(
    ...
    fun portfolioStatus(): String {
        return try {
            ...
            TelegramMessageFormatter.error("could not load portfolio data")
        }
    }
    ...
    fun allocationCheck(): String {
        return try {
            ...
            TelegramMessageFormatter.allocationCheck(drifts)
        } catch (e: Exception) {
            log.warn("allocationCheck failed: {}", e.message)
            TelegramMessageFormatter.error("could not load allocation data")
        }
    }
    ...
    fun topPerformers(): String {
        return try {
            ...
            val losers = sorted
                ...
                .takeLast(3)
                .map { it.symbol to it.pnlPercent }
            TelegramMessageFormatter.topPerformers(gainers, losers)
        } catch (e: Exception) {
            log.warn("topPerformers failed: {}", e.message)
            TelegramMessageFormatter.error("could not load performance data")
        }
    }
    ...
    fun watchlistQuery(): String {
        return try {
            ...
            TelegramMessageFormatter.error("could not load watchlist")
        }
    }
    ...
    fun stockAnalysis(symbol: String): String {
        return try {
            ...
            TelegramMessageFormatter.error("could not analyse $symbol")
        }
    }
    ...
    fun conceptQuestion(question: String): String {
        return try {
            ...
            TelegramMessageFormatter.error("could not answer your question right now")
        }
    }
}
